...
import { Menu, Network, LogOut } from 'lucide-react';
    ...
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/90 backdrop-blur dark:border-white/[0.06] dark:bg-black/60">
      <div className="page-container flex h-16 items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <Link
            to={session?.role ? getDashboardPath(session.role) : '/'}
        ...
        <div className="flex items-center gap-2">
          ...
          {session?.token ? (
            <button
              ...
              className="hidden items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 dark:text-zinc-400 dark:hover:bg-white/5 sm:inline-flex"
            >
              <LogOut className="h-4 w-4" />
              Logout
            </button>
          ) : (
            <Button as={Link} to="/signin" size="sm" className="hidden sm:inline-flex">
              Sign in
            </Button>
          )}
        </div>
